...
class Database:
    ...
    async def get_auction_participants(self, ad_id: int):
        """Получает список участников аукциона для объявления.
        Участники - это пользователи, которые добавили объявление в избранное или делали предложения по цене."""
        participants = set()
        
        # Добавляем пользователей, которые добавили объявление в избранное
        for user_id, favorite_ads in self.favorites.items():
            if ad_id in favorite_ads:
                participants.add(user_id)
        
        # Добавляем пользователей, которые делали предложения по цене
        offers = self.price_offers.get(ad_id, [])
        for offer in offers:
            participants.add(offer['user_id'])
