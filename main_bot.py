...
async def process_callback_ad(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    if action == 'description':
        ...
        if callback_query.from_user.id in ADMIN_IDS:
            # Админ может ввести цену вручную или использовать быстрые кнопки
            mode = ad.get('auction_mode', 'off')
            current_price = ad['price']
            
            keyboard = types.InlineKeyboardMarkup(row_width=2)
            keyboard.add(
                types.InlineKeyboardButton("+50K", callback_data=f"quick_price_{ad_id}_+50000"),
                types.InlineKeyboardButton("-50K", callback_data=f"quick_price_{ad_id}_-50000"),
            )
            keyboard.add(
                types.InlineKeyboardButton("+150K", callback_data=f"quick_price_{ad_id}_+150000"),
                types.InlineKeyboardButton("-100K", callback_data=f"quick_price_{ad_id}_-100000"),
            )
            keyboard.add(
                types.InlineKeyboardButton("Ввести вручную", callback_data=f"manual_price_{ad_id}"),
                types.InlineKeyboardButton("Отмена", callback_data=f"cancel_price_{ad_id}"),
            )
            
            await bot.send_message(
                callback_query.from_user.id,
                f"Текущая цена: {current_price} KZT\n\nВыберите действие:",
...
# Обработчик быстрого изменения цены (-50K, -100K, +50K, +150K)
@dp.callback_query_handler(lambda c: c.data and c.data.startswith('quick_price_'))
async def handle_quick_price_change(callback_query: types.CallbackQuery, state: FSMContext):
    if callback_query.from_user.id not in ADMIN_IDS:
        await callback_query.answer("У вас нет прав", show_alert=True)
        return
    ...
    try:
        ad_id = int(parts[2])
        change_str = parts[3]  # +100000 или -100000
    ...
    try:
        ...
        if new_price < 0:
            await callback_query.answer("Цена не может быть меньше 0", show_alert=True)
            return
        
        if new_price == old_price:
            await callback_query.answer("Цена не изменилась", show_alert=True)
            return
        ...
        direction = "увеличена" if change_amount > 0 else "уменьшена"
        await callback_query.answer(f"Цена {direction} до {new_price} KZT", show_alert=True)
        
        # Уведомляем всех о изменении цены
        updated_ad = await db.get_ad(ad_id)
        await notify_price_change(updated_ad, old_price, new_price, callback_query.from_user.id)
        
        # Обновляем карточку объявления
        await show_ad_with_navigation(callback_query, state, edit=True)
        
    except Exception as e:
        logger.error(f"Ошибка при быстром изменении цены: {e}")
        await callback_query.answer("Произошла ошибка", show_alert=True)

# Обработчик для ручного ввода цены
@dp.callback_query_handler(lambda c: c.data and c.data.startswith('manual_price_'))
async def handle_manual_price_input(callback_query: types.CallbackQuery, state: FSMContext):
    if callback_query.from_user.id not in ADMIN_IDS:
        await callback_query.answer("У вас нет прав", show_alert=True)
        return
    ...
    try:
        ...
        await bot.send_message(
            callback_query.from_user.id,
            "Введите новую цену или 'Отмена' для отмены:",
            reply_markup=cancel_keyboard()
        )
    except (ValueError, IndexError):
        await callback_query.answer("Некорректные данные", show_alert=True)

# Обработчик отмены изменения цены
@dp.callback_query_handler(lambda c: c.data and c.data.startswith('cancel_price_'))
async def handle_cancel_price_change(callback_query: types.CallbackQuery, state: FSMContext):
...
async def notify_price_change(ad, old_price: int, new_price: int, initiator_id: int):
    ...
    # Получаем участников аукциона (те, кто добавил в избранное или делал предложения)
    try:
        auction_participants = await db.get_auction_participants(ad['ad_id'])
    except Exception as e:
        logger.error(f"Ошибка при получении участников аукциона для объявления {ad['ad_id']}: {e}")
        auction_participants = []

    # Формируем текст уведомления для участников аукциона
    text_users = (
        f"🔔 Изменение цены на объявлении\n\n"
        f"Автомобиль: {ad['title']}\n"
        f"Цена {direction} на {diff_abs:,} KZT\n"
        f"Было: {old_price:,} KZT\n"
        f"Стало: {new_price:,} KZT"
    )

    # Отправляем уведомления только участникам аукциона
    for user_id in auction_participants:
        # Пропускаем администраторов и менеджеров (они уже получили уведомление)
        if user_id in recipients_admin:
            continue
        # Пропускаем инициатора изменения
        if user_id == initiator_id:
            continue
        try:
            await bot.send_message(user_id, text_users)
            logger.info(f"Уведомление об изменении цены отправлено участнику аукциона {user_id}")
        except Exception as e:
            logger.error(f"Не удалось отправить уведомление участнику аукциона {user_id}: {e}")
